...
conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/employees?useSSL=false","root","root");
...
try{
st.executeUpdate("insert into employees values("+id+",'"+name+"',"+salary+");");
}
catch(Exception e)
{
e.printStackTrace();
...
{
try{
st.executeUpdate("update employees set emp_salary='"+salary+"'where emp_id="+id);
}
catch(Exception e){
e.printStackTrace();}}}
